...
class Groupn extends AbstractRule
{
    ...
    protected function _calculate($rule, $item)
    {
        ...
        foreach ($allItems as $i => $allItem) {
            ...
            $quoteAmount = $this->priceCurrency->convert($rule->getDiscountAmount(), $allItem->getQuote()->getStore());
            if ($i < $countPrices - ($countPrices % $step)) {

                $discount = $itemPrice - $quoteAmount / $step;
                $originalDiscount = $itemOriginalPrice - $quoteAmount / $step;
                $baseDiscount = $baseItemPrice - $rule->getDiscountAmount() / $step;
                $baseItemOriginalDiscount = $baseItemOriginalPrice - $rule->getDiscountAmount() / $step;
