...
class CategoryPageEditActionControllerSaveObserver implements ObserverInterface
{
	...
	)
	{
		...
		$this->_listingBreakPoint = $this->_frontendHelper->getBreakpointM();
		$this->_xxsBreakPoint = $this->_frontendHelper->getBreakpointXXS();
	}
...
.products-list {
    .product-item {
        .product-item-details {
            .price-box {
                text-align: left !important; // $priceAlign
                .price-container {
                    .price-label {
                        display: none;
                    }
...
.products-list {
	...
	private function _generateToolbarCss($toolbarOptions)
	{
		...
		$select_border_width = strlen(trim($true)) ? 'border-width: ' . $selectOBW . ';' : 'border-width: 1px;';
