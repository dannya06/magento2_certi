...
class MenuRepository implements MenuRepositoryInterface
{
     ...
     * @throws \Magento\Framework\Exception\NoSuchEntityException
     */
    public function getByAlias($alias, $store = null, $customer_group_id = 0){
        $menu = $this->menuFactory->create();
        if($store && (is_numeric($store_id) || is_string($store))){
        ...
        if (!$menu->getId()) {
            throw new NoSuchEntityException(__('The Megamenu Profile with the "%1" ID doesn\'t exist.', $alias));
        }
        if (!$menu->getStatus()) {
