...
class Editor extends \Magento\Framework\App\Helper\AbstractHelper {
    ...
    /**
     * @var \Ves\Megamenu\Model\Config\Source\ChilCol
     */
    ...
    public function getSearchCategoriesOptionsHtml($keyword)
    {
        $categories = $this->storeCategories->getSearchCategoryIdsByName($keyword);
    ...
    public function renderCellTemplate($fieldName)
    {
        ...
        if (isset($field['type'])) {
            ...
            switch ($field['type']) {
                ...
                case 'category_search':
                    $html = '<input type="text" id="' . $id . '" class="' . $classes . '" data-bind="value: ' . $fieldName . '" placeholder="' . __( 'Search a category...' ) .'"/>';
                    break;
                case 'category_text':
                    $html = '<input type="text" id="' . $id . '" class="selected_category ' . $classes . '" data-bind="value: ' . $fieldName . '" disabled="disabled"/>';
                    $html .= '<div class="selected_category_results" style="display: none"></div>';
                    $html .= '<span class="category_text-remove"><i class="fa fa-refresh" aria-hidden="true"></i>&nbsp;' . __('Select...') . ' </span>';
                    break;
                case 'icon':
                ...
                case 'image':
                    ...
                    $buttonBlock = $this->_layout->createBlock(
                    ...
                    require([
                        "jquery"
                            ...
                            jQuery(document).ready(function($){
                                jQuery(document).on("click","#'.$buttonBlock->getId().'", function () {
                                    MediabrowserUtility.openDialog("' . $this->_backendData->getUrl('cms/wysiwyg_images/index',$fileBrowserUrlParams) . '", null, null,"' . $this->escaper->escapeQuote('Upload Image',true) . '", "")
                                });
                            })
                        })
                    </script>
                ...
                case 'editor':
                    ...
                    require([
                        "jquery"
                            ...
                            jQuery(document).ready(function($){
                                jQuery(document).on("click","#'.$buttonBlock->getId().'", function () {
                                    megamenuWysiwygEditor.open("'. $this->_backendData->getUrl('vesmegamenu/product/wysiwyg') . '", "' . $editorId . '", ' . json_encode($tinyMCEConfig) . ')
                                });
                            })
                        })
                    </script>';
        ...
        return '<input type="text"  id="' . $this->_getCellInputElementId('<%- _id %>',
                $fieldName) . '" name="' . $inputName . '" class="' . (isset($field['class']) ? $field['class'] : 'input-text') . '" ' . (isset($field['style']) ? ' style="' . $field['style'] . '"' : '') . ' />';
    }
