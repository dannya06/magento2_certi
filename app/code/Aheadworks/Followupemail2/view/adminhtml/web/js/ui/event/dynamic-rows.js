    ...
    return DynamicRows.extend({
        ...
        afterEmailFormUpdate: function (response) {
            ...
            if (!response.error) {
                ...
                if (eventFound) {
                    if (response.totals) {
                        this.source.data.events[eventIndex].totals = response.totals;
                    }
                    if (response.create) {
                        var recordData = this.recordData();
                        var index = recordData[eventIndex].emails.length;
                        email.record_id = index;
                        this.source.data.events[eventIndex].emails.push(email);
                        this.reload();
                    } else {
                        var emailFound = false;
                        var emailIndex = 0;
                        this.source.data.events[eventIndex].emails.forEach(function (oldEmail, index) {
