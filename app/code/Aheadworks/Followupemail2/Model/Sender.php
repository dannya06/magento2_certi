...
class Sender
{
    ...
    public function sendQueueItem(QueueInterface $queueItem)
    {
        ...
        try {
            ...
            } else if ($this->config->getBCCEmailAddresses()) {
                $bcc = $this->config->getBCCEmailAddresses();
            }
    ...
    public function renderEventQueueItem(EventQueueInterface $eventQueueItem, EmailContentInterface $emailContent)
    {
        ...
        if ($headerTemplate && $headerTemplate != EmailInterface::NO_TEMPLATE) {
            $header = $this->getHeaderTemplate($headerTemplate, $storeId, $emailData);
        }
        ...
        if ($footerTemplate && $footerTemplate != EmailInterface::NO_TEMPLATE) {
            $footer = $this->getFooterTemplate($footerTemplate, $storeId, $emailData);
        }
    ...
    public function getTestPreview($storeId, $emailContent)
    {
        ...
        if ($emailContent->getHeaderTemplate() && $emailContent->getHeaderTemplate() != EmailInterface::NO_TEMPLATE) {
            $header = $this->getHeaderTemplate($emailContent->getHeaderTemplate(), $storeId, $emailData);
        }

        $footer = '';
        if ($emailContent->getFooterTemplate() && $emailContent->getFooterTemplate() != EmailInterface::NO_TEMPLATE) {
            $footer = $this->getFooterTemplate($emailContent->getFooterTemplate(), $storeId, $emailData);
        }
     ...
     * @return string
     */
    private function getHeaderTemplate($templateId)
    {
        ...
        return $headerTemplate->getTemplateText();
    }
     ...
     * @return string
     */
    private function getFooterTemplate($templateId)
    {
        ...
        return $footerTemplate->getTemplateText();
    }
