...
use Aheadworks\Layerednav\Model\Layer\FilterListCategory;
...
/**
 * Test for \Aheadworks\Layerednav\Model\Layer\FilterListCategory
 */
class FilterListCategoryTest extends \PHPUnit\Framework\TestCase
{
    /**
     * @var FilterListCategory
     */
    private $filterListCategory;
    ...
    public function setUp()
    {
        ...
        $this->filterListCategory = $objectManager->getObject(
            FilterListCategory::class,
            [
    ...
    public function testGetFilters($filterType, $isValidForCategory, $attributeCode, $emptyResult)
    {
        ...
        $this->searchCriteriaBuilderMock->expects($this->once())
            ->method('addFilter')
            ->with(FilterInterface::IS_FILTERABLE, 0, 'gt')
            ->willReturnSelf();
        $this->searchCriteriaBuilderMock->expects($this->once())
            ->method('addSortOrder')
            ->with($sortOrderMock)
        ...
        if ($emptyResult) {
            $this->assertEquals([], $this->filterListCategory->getFilters($layerMock));
        } else {
            $this->assertEquals([$filterMock], $this->filterListCategory->getFilters($layerMock));
        }
    }
