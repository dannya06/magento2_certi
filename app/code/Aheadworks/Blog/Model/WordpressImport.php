...
class WordpressImport
{
    ...
    private function getPreparedPostData($post)
    {
        $postData = [
            ...
            PostInterface::AUTHOR_NAME          => $this->getPostAuthorName($post),
            PostInterface::STORE_IDS            => [\Magento\Store\Model\Store::DEFAULT_STORE_ID],
    ...
    /**
     * Get WP author name if exists, otherwise return current user name
     *
     * @param \SimpleXmlElement $post
     * @return string
     */
    private function getPostAuthorName($post)
    {
        $namespaces = $post->getNamespaces(true);
        $dcNamespaceItems = $post->children($namespaces['dc']);
        $postCreator = (string)$dcNamespaceItems->creator;

        if (array_key_exists($postCreator, $this->wpAuthors)) {
            return $this->wpAuthors[$postCreator];
        }
