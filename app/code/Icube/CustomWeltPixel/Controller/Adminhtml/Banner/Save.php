...
namespace Icube\CustomWeltPixel\Controller\Adminhtml\Banner;
        ...
        if ($data) {
            ...
            if (!$bannerId) {
                ...
                foreach ($data['slider_id'] as $sliderId) {
                    ...
                    $tblSalesOrder = $connection->getTableName('weltpixel_owlcarouselslider_banners');
                    $result = $connection->fetchAll("SELECT * FROM ".$tblSalesOrder." WHERE slider_id LIKE '%$sliderId%' ORDER BY id DESC LIMIT 1");

                    foreach ($result as $banner) {
                        $sortOrder = @unserialize($banner['sort_order']);

                        if ($sortOrder !== false && isset($sortOrder[$sliderId])) {
                            $currentSortOrder = $sortOrder[$sliderId];
                        } else {
                            $currentSortOrder = (int)$banner['sort_order'];
                        }
                    }

                    $arrSortOrder[$sliderId] = $currentSortOrder+1;
                }

                if (count($data['slider_id']) > 1) {
                    $sortOrder = @serialize($arrSortOrder);
                } else {
