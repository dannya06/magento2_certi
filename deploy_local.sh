...
case "$1" in
	...
	echo "$CASE_H"
		...
		printf "  bash deploy.sh --full\n\n"
		printf "Deployment with all options enabled WITH composer install:\n"
		printf "  bash deploy.sh --composer-install\n\n"
		printf "Deployment with all options enabled WITHOUT composer install AND setup upgrade:\n"
		printf "  bash deploy.sh --no-setup-upgrade\n\n"
		printf "Static content deploy only:\n"
		printf "  bash deploy.sh --static-deploy-only\n\n"
		printf "Deployment with fixing file & folder permission in pub/ & var/log/ directory:\n"
		printf "  bash deploy.sh --fix-permission --full\t\t OR\n"
		printf "  bash deploy.sh --fix-permission --composer-install\t OR\n"
		printf "  bash deploy.sh --fix-permission --no-setup-upgrade\t OR\n"
		printf "  bash deploy.sh --fix-permission --static-deploy-only\n\n"
		exit 0 ;;
